...
//** mui */
import { alpha, IconButton, Table, TableBody, TableContainer, Paper, TableRow, TableHead, TableCell, Backdrop, Box, Button, Card, Checkbox, Chip, CircularProgress, Container, Divider, FormControl, InputLabel, MenuItem, Select, Stack, TablePagination, TextField, Toolbar, Tooltip, Typography, Grid } from '@mui/material';
...
import SingleConsumerEditModal from '../../components/consumerRegistration/SingleConsumerEditModal';

const ConsumerList = () => {

    const { userId, orgId, isAdmin, orgName } = useSelector((state) => state.user);
    ...
    const isSelected = (name) => activeSelected.indexOf(name) !== -1;
    ...
    const consumerFilter = async () => {
        ...
        await consumerListByFilter(filterBody).then((resp) => {
            ...
            console.log("🚀 ~ file: consumerList.js:162 ~ await consumerListByFilter ~ error:", error)
        }).finally(() => {
            hideLoader()
        })
    ...
    const handleClick = (event, name) => {
        const selectedIndex = activeSelected.indexOf(name);
        let newSelected = [];
        if (selectedIndex === -1) {
            newSelected = newSelected.concat(activeSelected, name);
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            setActiveSelected(consumerList);
            return;
        }
        setActiveSelected([]);
    };
    ...
    const EnhancedTableToolbar = (props) => {
        ...
        const allConsumer = numSelected.length;
        const activateConsumer = numSelected?.filter(e => e.Active > 0).length
        const deactivateConsumer = numSelected?.filter(e => e.Active == 0).length
                    ...
                    <>
                        {activateConsumer > 0 ?
                            <Typography sx={{ flex: '1 1 100%' }} color="inherit" variant="subtitle1" component="div"  >
                                {activateConsumer} active consumer
                                <IconButton onClick={() => {
                                    if (isAdmin) {
                                        alertDialog({
                                            desc: "Are you sure you want to deactivate the consumer?",
                                            rightButtonText: "Okay",
                                            rightButtonFunction: deactivateConsumerStatus,
                                        })
                                    } else {
                                        openAlertHandle('error', "Only users with admin access can modify the consumer.")
                                    }
                        ...
                        {deactivateConsumer > 0 ?
                            <Typography sx={{ flex: '1 1 100%' }} color="inherit" variant="subtitle1" component="div" >
                                {deactivateConsumer} deactivate consumer
                                <IconButton onClick={() => {
                                    if (isAdmin) {
                                        alertDialog({
                                            desc: "Are you sure you want to activate the consumer?",
                                            rightButtonText: "Okay",
                                            rightButtonFunction: activeConsumerStatus,
                                        })
                                    } else {
                                        openAlertHandle('error', "Only users with admin access can modify the consumer.")
                                    }
                        ...
                        <Typography sx={{ flex: '1 1 100%' }} color="inherit" variant="subtitle1" component="div" >
                            {allConsumer} selected consumer
                            {/* <IconButton onClick={() => {
                                if (isAdmin) {
                                    alertDialog({
                                        desc: "Are you sure you want to delete the consumer?",
                                        rightButtonText: "Okay",
                                        rightButtonFunction: deleteConsumerStatus,
                                    })
                                } else {
                                    openAlertHandle('error', "Only users with admin access can modify the consumer.")
                                }
    ...
    const deactivateConsumerStatus = async () => {
        ...
        const body = {
            ...
            "data": activeSelected.map(e => e.Active != 0 && e.ID),
            "active": 0,
            "orgId": orgId
        }
        await consumerManageAPI(body).then((resp) => {
            ...
            console.log("🚀 ~ file: consumerList.js:285 ~ await consumerManageAPI ~ error:", error)
        }).finally(() => {
            consumerFilter()
            showLoader()
        });
    ...
    const activeConsumerStatus = async () => {
        showLoader();
        try {
            const body = {
                "type": "CONSUMER_STATUS_UPDATE",
                "data": activeSelected.map(e => e.fetchType == 0 && e.ID),
                "active": 1,
                "orgId": orgId
            }
            const nonFetchBody = {
                "type": "CONSUMER_STATUS_UPDATE",
                "data": activeSelected.map(e => e.fetchType == 1 && e.ID),
                "active": 2,
                "orgId": orgId
            }
            const fetchResp = await consumerManageAPI(body);
            const nonFetchResp = await consumerManageAPI(nonFetchBody);
            openAlertHandle('success', fetchResp?.data + " \n" + nonFetchResp?.data);
        } catch (error) {
            console.log("🚀 ~ file: consumerList.js:311 ~ activeConsumerStatus ~ error:", error)
        } finally {
            setActiveSelected([])
            consumerFilter()
            showLoader()
    ...
    const deleteConsumerStatus = async () => {
        showLoader();
        try {
            const body = {
                "type": "CONSUMER_DELETE",
                "data": activeSelected.map(e => e.ID),
                "active": 1,
                "orgId": orgId
            }
            const resp = await consumerManageAPI(body);
            if (resp?.statusCode === 200) {
                openAlertHandle('success', resp?.data);
            }
    ...
    const handleChangeStateName = async (event) => {
        ...
        await billerNameRequest(BILLER_CATEGORY, event.target.value).then((data) => {
            if (data) {
    ...
    const ConsumerRow = ({ item, index }) => {
        ...
        return (<StyledTableRow>
            ...
            <TableCell align="center"><Chip label={consumerStatus[item.Active]} color={consumerStatusColor[item.Active]} variant="outlined" /> </TableCell>
            ...
            <SingleConsumerEditModal clearState={clearState} data={item} open={isEditModal} onClose={closeEditModal} />
    ...
    return (
        <>
            <Head><title>Consumer List</title></Head>
            <Stack component="main" spacing={3} p={3} >
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: "center" }} >
                    <Box>
                        ...
                        <CButton onClick={() => goToPage('/admin/consumerRegistration')} title="Consumer Registration" />
                    </Box>
                    <BBPSLogo />
                </Box>
                <Card>
                ...
                <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                    {activeSelected.length > 0 ? <EnhancedTableToolbar numSelected={activeSelected} /> : null}
                    <TableContainer>
                            ...
                            <TableHead>
                                <TableRow>
                                    <TableCell padding="checkbox">
                                        <Checkbox
                                            color="primary"
                                            indeterminate={activeSelected.length > 0 && activeSelected.length < consumerList.length}
                                            checked={consumerList.length > 0 && activeSelected.length === consumerList.length}
