...
//** mui */
import { Box, Container, TextField, MenuItem, FormControl, Select, InputLabel, Typography, Button, Stack, Card, Divider, Grid, Chip } from '@mui/material';
...
const PDFParameterReport = () => {
    ...
    const getSingleReport = async () => {
        ...
        await reportByFilter({
            "type": "SingleReports",
            "data": filterBody,
            "reportType": "PDF_PARAMETER_REPORT"
        }).then((resp) => {
            if (resp !== undefined) {
            ...
            console.log("🚀 ~ file: PDFParameterReport.js:42 ~ getSingleReport ~ error:", error)
        }).finally(() => {
            hideLoader()
        })
    }
...
export default PDFParameterReport;
