...
const ConsumptionAverageTab = () => {
    ...
    const getSingleReport = async () => {
        ...
        await reportByFilter({
            "type": "SingleReports",
            "data": filterBody,
            "reportType": "BILL_AVERAGE_CONSUMPTION"
        }).then((resp) => {
            if (resp !== undefined) {
            ...
            console.log("🚀 ~ file: ConsumptionAverageTab.js:43 ~ getSingleReport ~ error:", error)
        }).finally(() => {
            hideLoader()
        })
    }
                                ...
                                {billAverageList && billAverageList.map((row, index) => {
                                    return (
                                        <StyledTableRow key={String(index) + String(row.ConsumerId)} >
                                            ...
                                            <TableCell align="center">{isEmpty(Number(row?.rollingAverage).toFixed(2))}</TableCell>
                                            <TableCell align="center">{isEmpty(Number(row?.threeMonthAvg).toFixed(2))}</TableCell>
...
export default ConsumptionAverageTab;
