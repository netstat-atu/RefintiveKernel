...
const NotProcessBillTab = () => {
    ...
    const handleChangeOrganization = useCallback(async (event) => {
        ...
        setFilterBody({ ...filterBody, "orgId": organizationId });
    }, []);

    const renderedOrganizationList = useMemo(() => {
        if (organizationList && organizationList.length > 0) {
            return organizationList.map((val, index) => (
                <MenuItem key={`${val.orgId}`} value={val.orgId + "/" + val.name}>
    ...
    const downloadExcel = async () => {
        try {
            ...
            const body = {
                "type": "CREATE",
                "id": id,
                "name": excelName.NOT_PROCESS_BILL,
                "data": filterBody,
                "orgId": orgId,
                "userId": userId
            }
    ...
    const clearState = () => {
        ...
        setOrganizationIdName(orgId + "/" + orgName);
...
export default NotProcessBillTab