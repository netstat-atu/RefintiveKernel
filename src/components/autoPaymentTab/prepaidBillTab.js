...
//** mui */
import { Button, Table, TableHead, Paper, TableRow, TableContainer, TableCell, TableBody, Box, Card, Divider, Stack, TextField, Typography, Grid, Select, FormControl, InputLabel, MenuItem, TablePagination, Chip, Checkbox } from '@mui/material';
import { DatePicker } from '@mui/x-date-pickers/DatePicker';

//** service */
import { adminAutoPaymentManage, adminAutoPrepaidBillPayment, billerNameRequest, billerNameStates, downloadExcelFromAPI, getAutoBillFilter } from '../../utils/bbps/billerServices';
...
const PrepaidBillTab = () => {
    ...
    const initiatePayment = async () => {
        //**  statusPayment */
        showLoader()
        if (filterBody.batchId) {
            const body = {
                "batchId": filterBody.batchId,
            };
            await adminAutoPrepaidBillPayment(body).then((resp) => {
                openAlertHandle('success', "Auto prepaid bill payment initiation was successful. You will receive an email, and after that, you can initiate your next payment.")
            }).catch((error) => {
                console.log("🚀 ~ file: ProcessBillTab.js:94 ~ await adminAutoPaymentManage ~ error:", error)
            })
        } else {
            openAlertHandle("error", "Please choose batchId");
        }
        hideLoader()
    ...
    return (
        <Stack spacing={3} pt={3}>
            <Typography variant='body2'>
                {`NOTE : Prepaid bills are not supported by the API.`}
            </Typography>
            <Card>
                ...
                <Stack>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} justifyContent={"space-between"} p={1} >
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} >
                            <Button disabled={isLoading} onClick={getPrepaidBill} variant="contained" >Search</Button>
                            <Button onClick={clearState} variant="contained" color='inherit'>Clear</Button>
                            {filterBody.orgId && filterBody.batchId ? <Button disabled={isLoading} onClick={statusPayment} variant="contained" >Initiate Payment</Button> : null}
                        </Stack>
