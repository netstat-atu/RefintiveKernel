...
//** service */
import { billerNameRequest, billerNameStates, getAutoBillFilter } from '../../utils/bbps/billerServices';
...
const NonFetchBillTab = () => {
    const { orgName, orgId, organizationList, userId } = useSelector((state) => state.user);
    ...
    const getNonFetchBill = async () => {
        ...
        try {
            ...
            console.log("🚀 ~ file: processBillTab.js:279 ~ getNonFetchBill ~ error:", error)
        } finally {
            hideLoader()
        }
    ...
    return (
        <Stack spacing={3} pt={3}>
            <Typography variant='body2' marginBottom={2}>
                {`NOTE : Non Fetch bills that cannot be retrieved using the API are not supported.`}
            </Typography>
            <Card>
                ...
                <Stack>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} justifyContent={"space-between"} p={1} >
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} >
                            <Button disabled={isLoading} onClick={getNonFetchBill} variant="contained" >Search</Button>
...
export default NonFetchBillTab